"""003 — Same model + features as 002, but a long enough budget to converge.

002 hit the 5000-round cap with val RMSE still falling. Push the cap to
20000 with patience 200 to find the natural early-stop point. Holding
everything else fixed isolates the convergence effect.
"""
...
def main() -> None:
    ...
    with run(
        "lightgbm-features-long",
        description="002 with 20k rounds / patience 200 to hit natural early stop.",
    ...
    print(f"best_iteration = {booster.best_iteration}  fit_seconds = {fit_seconds:.1f}")
    for k, v in metrics.items():
        print(f"  val.{k}: {v:.6f}")
